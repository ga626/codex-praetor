...
try {
    ...
    Invoke-Check "provider release hard gate" (Join-Path $verify "test-provider-release-evidence.ps1") @("-EvidencePath", (Join-Path $root "config\provider-release-evidence.json"), "-ArtifactManifestPath", $artifactPath)
