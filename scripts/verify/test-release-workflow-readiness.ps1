...
Assert-True ($pipelineText -match "steps\.release_impact\.outputs\.release_impact\s*==\s*'true'") "Mainline publication must run only for release-impact changes."
Assert-True ($pipelineText -match '(?ms)- name: Materialize release PR evidence\s*\r?\n\s*if:.*steps\.release_impact\.outputs\.release_impact') "Release evidence must be required only for a release-impact candidate."
Assert-True ($pipelineText -match '(?ms)- name: Validate non-release MCP dependency update\s*\r?\n\s*if:.*release_impact.*!=.*true.*\r?\n\s*shell: pwsh\s*\r?\n\s*run:.*install-mcp-dependencies\.ps1.*npm --prefix \.\\mcp test') "Non-release dependency candidates must install and test MCP dependencies instead of requiring release evidence."
Assert-True ($pipelineText -match '(?ms)- name: Install MCP dependencies for promoted artifact verification\s*\r?\n\s*if:.*steps\.release_impact\.outputs\.release_impact.*\r?\n\s*shell: pwsh\s*\r?\n\s*run:.*install-mcp-dependencies\.ps1') "Main promotion must install the controlled MCP test dependencies before final artifact runtime verification."
