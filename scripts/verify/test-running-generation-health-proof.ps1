...
try {
    ...
    $entry = [ordered]@{
        ...
        provider = "codebuddy"; distribution = "codebuddy_cn"; cli_path = $cli; cli_hash = $cliHash; model = "hy3"; permission_profile = "local-audit-v1"; task_kind = "local_audit"; connection_mode = "codebuddy_acp"
    ...
    [ordered]@{
        schema = "codex-praetor-generation-readiness/v5"; status = "passed"; generation_id = [string]$generation.generation_id
    ...
    [ordered]@{
        schema = "codex-praetor-generation-readiness/v5"; status = "passed"; generation_id = "new-package-generation"
        runtime_contract_sha256 = $contractHash; task_contract_schema = [string]$contract.taskContractSchema; entries = @($entry)
    } | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath (Join-Path $profile ".codex\codex-praetor-readiness.json") -Encoding UTF8
