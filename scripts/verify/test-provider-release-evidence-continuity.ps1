...
try {
    ...
    New-Item -ItemType Directory -Path (Join-Path $fixture "plugin\mcp\dist") -Force | Out-Null
    Invoke-Git @("init", "-q")
    ...
    [IO.File]::WriteAllText((Join-Path $fixture "plugin\mcp\dist\qoder-sdk-runner.js"), "// qoder runner baseline`n", (New-Object Text.UTF8Encoding($false)))
    ...
    [IO.File]::AppendAllText((Join-Path $fixture "plugin\mcp\dist\qoder-sdk-runner.js"), "// qoder SDK behavior changed`n", (New-Object Text.UTF8Encoding($false)))
    Invoke-Git @("add", ".")
    Invoke-Git @("commit", "-qm", "qoder provider behavior")
    $qoderTarget = ((& git -C $fixture rev-parse HEAD | Out-String).Trim())
    $qoderImpact = @(Get-CodexPraetorProviderCompatibilityImpact -Repo $fixture -ComparisonBase $versionTarget -TargetRef $qoderTarget)
    Assert-True (($qoderImpact -join ",") -eq "qoder") "A Qoder-only adapter change must not demand CodeBuddy evidence merely because the runtime contract version changed."
    $qoderSurface = Get-CodexPraetorProviderCompatibilitySurfaceHash -Repo $fixture -Revision $qoderTarget
    ...
    $observabilityImpact = @(Get-CodexPraetorProviderCompatibilityImpact -Repo $fixture -ComparisonBase $qoderTarget -TargetRef $observabilityTarget)
    Assert-True ($observabilityImpact.Count -eq 0) "runtime_info-only changes must not demand provider evidence."
    Assert-True ($qoderSurface -eq (Get-CodexPraetorProviderCompatibilitySurfaceHash -Repo $fixture -Revision $observabilityTarget)) "runtime_info-only changes must preserve the provider compatibility surface hash."
    ...
    Write-Output "[PASS] Provider evidence continuity preserves version-only releases, scopes a Qoder-only change to Qoder, and rejects shared provider behavior changes."
} finally {
