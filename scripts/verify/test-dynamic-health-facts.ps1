...
try {
    ...
    $entry = [ordered]@{ generation_id = "gen-1"; runtime_contract_sha256 = "contract-1"; task_contract_schema = "task-v4"; provider = "codebuddy"; cli_path = $cli; cli_hash = $cliHash; model = "hy3"; permission_profile = "local-audit-v1"; task_kind = "local_audit"; connection_mode = "codebuddy_acp"; runner_identity = "fixture-runner"; status = "passed"; expires_at = (Get-Date).AddHours(1).ToString("o"); evidence = $evidence }
