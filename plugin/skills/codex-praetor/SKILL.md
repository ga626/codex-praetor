...
- 用户直接说“开启/打开/进入 Codex 执行官模式”或“开启执行官模式”时，先调用 `codex_praetor_mode_enable`。讨论“执行官模式是什么”不触发开启。
...
- 用户直接说“关闭/退出/停止 Codex 执行官模式”或简称“关闭执行官模式”时，调用 `codex_praetor_mode_disable`。它只取消当前对话、当前项目、当前模式会话名下的活跃 worker，并取得终态后才关闭。
