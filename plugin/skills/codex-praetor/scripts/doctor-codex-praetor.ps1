...
function Test-Provider {
    ...
    if ($versionCommand.Count -eq 0) {
        ...
        if ($versionProbe.returncode -eq 0) {
            ...
            Add-Check "provider:${Provider}:version" "info" "$Provider version probe did not prove compatibility. Exit code: $($versionProbe.returncode)." "Complete the provider's official login, then start a real plan task; the first-use bootstrap runs automatically. A readonly canary remains available for diagnosis. See $(Get-ProviderSetupDoc -Provider $Provider)."
        }
    }

            Add-Check "provider:${Provider}:auth" "info" "$Provider CLI presence is checked, but login/account state is intentionally not inspected by doctor." "Complete the provider's normal login flow outside Codex Praetor, then start a real plan task; Codex Praetor will perform the bounded first-use bootstrap. See $(Get-ProviderSetupDoc -Provider $Provider)."
}
