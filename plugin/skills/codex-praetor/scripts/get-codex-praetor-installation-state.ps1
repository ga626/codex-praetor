...
if (Test-GenerationEqual $expected $installed -and $marketplaceOk) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($HostRuntimeInfoPath)) {
        ...
        if ($hostMatches) {
            $status = "needs_first_use_bootstrap"
...
$payload = [ordered]@{
    ...
    next_action = switch ($status) {
        ...
        default { "Complete provider login, then start one real plan task; Codex Praetor will perform the bounded first-use bootstrap automatically." }
    }
}
