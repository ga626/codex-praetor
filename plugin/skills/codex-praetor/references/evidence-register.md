...
- Bundled CodeBuddy product configs list `hy3-preview-agent` as `Hy3 preview` with `x0.04 credits`, plus `deepseek-v4-flash` with `x0.06 credits` and `deepseek-v4-pro` with `x0.16 credits`. Tencent Cloud TokenHub CodeBuddy Code official setup uses model id `hy3` in `models.json` and `availableModels`, and the 2026-07-08 usage page showed `hy3` calls consuming 0 credits.
...
| Which Hy3 name should be the default: `hy3` or `hy3-preview-agent`? | Local 2026-07-07 smoke evidence shows `hy3` returned `CODEBUDDY_OK`. Tencent Cloud TokenHub CodeBuddy Code official setup uses `hy3` as the model id and `availableModels` entry. User-provided 2026-07-08 CodeBuddy usage detail shows `hy3` rows consuming 0 credits, while `hy3-preview-agent` rows consume 0.09/0.17 credits. Bundled product configs also mark `hy3-preview-agent` as `x0.04 credits`. | `hy3` is the correct default free route. `hy3-preview-agent` is a paid preview route and should not be the default. | `codebuddy-free` now uses `hy3`; `hy3-preview-agent` remains allowed only as a paid fallback. |
...
| `codebuddy-free` was temporarily changed to `auto`, then incorrectly changed to DeepSeek-V3.2, then to `hy3-preview-agent`. | Local `codebuddy --help` treats `auto` as a supported model choice, not a fixed backend. DeepSeek-V3.2 was never selected as the preferred Tencent model. Later `hy3-preview-agent` looked canonical in bundled configs, but those configs also mark it as `x0.04 credits`, and real usage showed nonzero consumption. The earlier and official free route is `hy3`. | `auto`, DeepSeek-V3.2, and `hy3-preview-agent` are all wrong defaults for the user's selected free Tencent route. | `codebuddy-free` now uses fixed `hy3`; `auto` remains in `blockedModels`; the wrapper rejects `auto` unless `-AllowAutoModel` is intentionally passed. DeepSeek-V3.2 is not a default/recommended tier. |
