...
Date: 2026-07-17
Target release: `0.4.1-alpha`

Status: `v0.4.0-alpha` is already published. `v0.4.1-alpha` must merge before its new tag and Release are created from the latest `main`; existing tags are immutable.
...
7. After the release PR is merged, return to the latest clean `main`. Preview the publication first, then publish:
   ...
   git switch main
   git pull --ff-only
   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\release\publish-github-release-asset.ps1 -Version 0.4.1-alpha -Tag v0.4.1-alpha
   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\release\publish-github-release-asset.ps1 -Version 0.4.1-alpha -Tag v0.4.1-alpha -Apply
   ```
