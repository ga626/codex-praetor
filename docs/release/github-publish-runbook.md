...
Date: 2026-07-19
Target release: `0.8.2-alpha`

Status: `v0.6.2-alpha` is a public release incident and must not be activated or overwritten. `v0.8.2-alpha` is the explicit recovery version and is published automatically by `Release On Main` after this PR reaches `main`.
...
8. 远端下载复验通过后，Codex 在本机执行同一 Release 的自动激活：
   ...
   ```powershell
   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\release\activate-published-codex-praetor-release.ps1 -Version 0.8.2-alpha -Json
   ```
