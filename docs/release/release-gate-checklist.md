...
- Provider release hard gate: after the final artifact is verified, `scripts/verify/test-provider-release-evidence.ps1` must pass against `config/provider-release-evidence.json`. The manifest must bind the current HEAD and artifact SHA and contain one Codex-accepted real task plus an accepted canary for both Qoder (`qoder_agent_sdk`) and CodeBuddy (`codebuddy_acp`). A synthetic fixture, exit code 0 without acceptance, stale generation, or one-provider-only evidence blocks merge and publication.
...
- Current release notes: `docs/release/release-notes-0.16.22-alpha.md`.
