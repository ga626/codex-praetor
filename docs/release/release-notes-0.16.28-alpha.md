...
- Qoder China code-change workers now use Qoder's `accept_edits` permission mode inside the already isolated worktree; read-only and test tasks remain fail-closed with `dont_ask`.
