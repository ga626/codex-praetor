...
2. `docs/installation.zh.md` 已有 Windows 安装说明。
3. `docs/troubleshooting.zh.md` 已区分插件不可见、MCP 工具不可见、`Transport closed`、provider 缺失等问题。
4. `docs/privacy.zh.md` 已说明不读取 token、cookie、账号数据库和本机私有配置。
5. `docs/provider-notes/qoder.md`、`docs/provider-notes/codebuddy.md`、`docs/provider-notes/mimo.md` 已存在。
6. `scripts/install-user.ps1` 已有用户安装脚本。
7. `scripts/doctor-codex-praetor.ps1` 已有 provider 检测和状态提示基础。
8. `scripts/build-codex-praetor-release.ps1` 已有发布包构建和部分私有痕迹排除逻辑。
9. `plugin/` 已有插件包形态。
10. `mcp/` 已有 MCP 源码和 self-test。
11. `docs/release-gate-checklist.md`、`docs/fresh-context-native-mcp-canary.md`、`docs/user-acceptance-checklist.zh.md` 已覆盖发布和验收思路。
...
2. 复用现有 `scripts/install-user.ps1`。
3. 启动器打开后显示中文标题和中文向导。
4. 等流程稳定后再考虑小型 GUI。
...
8. `docs/troubleshooting.zh.md`  
   按“用户看到的现象”重排。

9. `docs/release-notes-0.1.0-alpha.md`  
   改成中文 Release 主说明，明确下载资产。

10. `scripts/build-codex-praetor-release.ps1`  
    后续实现时加强公开痕迹扫描，但本轮不修改。

11. 新增安装入口  
    后续新增 `setup.cmd` 和 `setup.ps1`。文件名用 ASCII，窗口标题和正文用中文。第一版调用现有 `scripts/install-user.ps1`，不复制主安装逻辑。
