# Codex Praetor 文档入口

普通用户优先看：

- [安装指南](user/installation.zh.md)
- [排错指南](user/troubleshooting.zh.md)
- [卸载、更新和回滚](user/uninstall.zh.md)
- [隐私与本地数据边界](user/privacy.zh.md)
- [用户验收清单](user/user-acceptance-checklist.zh.md)
- [provider 说明](provider-notes/README.md)

开发者和维护者看：

- [架构说明](architecture/architecture.md)
- [Fresh-context native MCP canary](architecture/fresh-context-native-mcp-canary.md)
- [路线图](roadmap.md)
- [Release gate checklist](release/release-gate-checklist.md)
- [GitHub publish runbook](release/github-publish-runbook.md)
- [Release notes 0.2.0-alpha](release/release-notes-0.2.0-alpha.md)
- [0.5.0-alpha Desktop runtime identity 与发布证明 PR 说明](release/release-notes-0.5.0-alpha.md)
- [Release notes 0.1.2-alpha](release/release-notes-0.1.2-alpha.md)
- [Release notes 0.1.1-alpha](release/release-notes-0.1.1-alpha.md)
- [Release notes 0.1.0-alpha](release/release-notes-0.1.0-alpha.md)

历史调研和交付报告放在 [reports/](reports/)。这些文档保留项目决策的来龙去脉，但普通安装、排错和发布时不需要从这里开始读。
