...
但它还不能直接发布到 GitHub。当前最大差距不是 MCP 是否存在，而是产品化边界还没收紧：仓库没有首个提交，真实 worker worktree 无法创建；示例配置和证据文件里仍有本机路径、账号、额度观察；README、安装说明、doctor、许可证、插件元数据和外部 Agent 缺失时的用户体验还不够公开发布。

正确路径是：先清理发布包和隐私边界，做首个干净提交，然后跑一次真实 MiMo readonly canary，再补 doctor 和安装说明，最后做 GitHub alpha 发布。
...
- `C:\Users\ga990\.codex\skills\codex-praetor` 是本机安装版 skill，并且是实目录，不是链接。
...
当前还是 `No commits yet on master`，所有文件都是未跟踪状态。

这会直接影响真实 worker dispatch。MiMo readonly 也需要工作在 Codex 创建的 git worktree 里，因为 MiMo 的 plan 模式仍可能写 `.mimocode` 计划文件。没有首个提交时，Git 没有可检出的基线，worktree 不能包含当前项目内容。

人话说：现在 dry-run 能跑，MCP 能测，脚本能构造命令；但真实 worker 还不能审计这个项目，因为这个项目在 Git 眼里还没有一个稳定快照。
...
- 插件 manifest 仍有占位信息：repository/homepage 指向 OpenAI 插件文档，license 仍是 `UNLICENSED`。
...
- 确定许可证。公开 alpha 建议至少选 MIT / Apache-2.0 / source-available 中一种，不要继续 `UNLICENSED`。

完成标准：全文搜索本机路径、账号、token/key、截图路径，无公开泄漏；插件 manifest 不再用占位 URL。
