...
```text
https://github.com/ga626/codex-praetor/releases/tag/v0.16.34-alpha
```
...
真实派工前，你需要至少一个外部 CLI 已安装并按 provider 官方方式完成授权。`0.16.34-alpha` 对中国版 Qoder 使用官方 CLI 的 `--print --output-format stream-json`，对 CodeBuddy 使用 ACP；你不需要手工选择连接协议，也不需要为了首次使用手动跑内部 canary。全球 Qoder Agent SDK 不会自动启用，只有另行安装并完成专门验证后才是可选路线。
