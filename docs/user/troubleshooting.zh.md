...
| 更新后 health 提示没有 readiness | 当前机器没有与所选 CLI、模型、权限、连接和 runner 指纹匹配的有效证据 | 直接提交一个真实计划任务；系统会在隔离 worktree 中自动做一次首用 bootstrap。若失败，按错误分类处理；不要编辑 `active.json` 或 readiness 文件 |
...
先确认你已经运行过安装向导。如果你使用的是 `0.16.24-alpha` 的 Windows 安装 zip，优先直接双击根目录的 `setup.cmd`。自动化或排错时也可以运行：
...
如果你在 Codex 对话里，先确认插件运行时身份，再按 Skill 的派工步骤执行：
