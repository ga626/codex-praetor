...
> 用户从 GitHub 下载 Codex Praetor，双击一个入口，按中文向导完成本体安装；可以选择全部配置 provider、全部不配置，或只配置 Qoder / CodeBuddy / MiMo 中任意一个；涉及账号、扫码、浏览器授权、Token Plan、API key 的步骤由用户本人在官方流程中完成；向导负责等待、复检、记录本机路径、运行只读验收，并给出最终状态总览。
...
如果用户选择某个 provider，向导先判断它是否已经安装。已安装就显示版本和下一步授权提示；未安装就给出官方安装方式，并让用户选择“打开官方说明 / 执行官方安装命令 / 跳过”。这里不引入第三方镜像，不把 provider 打进 Codex Praetor 包，也不静默安装 Node 或 provider。
...
用户点继续后，向导重新检测命令和版本；如果用户愿意，继续跑只读 canary。canary 的意义要讲人话：
...
路线图要移除或改写“不会安装 provider”这种容易误解的话。正确说法是：不会替用户登录、不会读取凭据、不会静默安装或越过官方流程；但会提供安装引导、等待授权、复检和 canary。
...
安全边界是：Codex Praetor 只保存本机 CLI 路径、用户选择和跳过状态；不保存 token、cookie、PAT、API key、账号数据库、余额页面或截图。需要密钥的 provider，由 provider 自己保存；Codex Praetor 只调用 provider 官方 CLI。
...
- 选择 provider 后，向导会给官方安装入口，提示用户完成官方登录、浏览器授权、站点选择、企业域、Token Plan 或 API key 等账号动作，然后回到向导复检。
...
- Provider 页面统一说明：Codex Praetor 会引导官方安装/登录、等待用户完成、复检和记录 CLI 路径，但不会静默安装、代登录或读取凭据。
...
这次 PR 不需要做 GUI，不需要做 exe，不需要静默安装 Node 或 provider，不需要读取任何 provider 凭据。文本向导足够，但它必须是完整向导，而不是提示菜单。
