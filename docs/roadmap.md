...
`v0.9.1-alpha` 是当前已公开交付的基线。候选 `v0.9.2-alpha` 在评测合同之上增加保守、可解释的派工建议和失败恢复：只有通过当前硬门且具有同任务族新鲜证据的完整 provider tuple 才会被推荐；建议不自动派工、合并或发布。合并后仍只由 Release On Main 创建、发布和复验不可变 Release；本机验证严格区分隔离开发 profile 与已安装用户 runtime。
