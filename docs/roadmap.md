...
`v0.1.1-alpha` 是当前已发布给普通用户下载的版本。源码线已经进入 `0.1.2-alpha` 预发布候选阶段，重点从“安装和 dry-run 可用”推进到“Codex 可以真实派发外部 worker、收集结果、记录验收结论，并按计划继续下一步”。
...
- 0.1.2-alpha 发布收口：重新构建 Release zip、上传 `.sha256`、下载远端 zip 复验、确认新 MCP 工具在普通用户入口可见。
