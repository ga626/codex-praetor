...
`v0.1.1-alpha` 是当前发布收口版本。公开仓库、安装体验、provider 配置引导、排错路径、运行态目录和发布包边界已经进入可发布、可安装、可反馈问题的状态。当前重点不是继续堆新功能，而是保持这些用户路径一致，并修掉发布后暴露出的文档、缓存恢复和真实派工体验细节。
...
- 更友好的插件缓存清理和恢复提示。
- provider 真实派工失败后的恢复路径，例如更细的登录、余额、权限、模型不可用分类。
