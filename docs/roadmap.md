...
`v0.9.3-alpha` 是当前已公开交付的基线，但已确认存在“已安装运行时缺少 provider 运营数据”的发布事故。候选 `v0.10.0-alpha` 只做该事故的不可变修复：把 adapter 合同和准入清单放进真正安装的插件，并在最终 bundled MCP 中实际调用验证。它不会读取认证材料，也不会自动扩大路由；合并后仍只由 Release On Main 创建、发布和复验不可变 Release。
