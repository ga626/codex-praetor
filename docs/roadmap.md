...
`v0.8.2-alpha` 已合并但未完成产品交付：真实 CodeBuddy 编辑 worker 被失效权限参数拒绝，必须以新的 `v0.8.3-alpha` 恢复版本修复。发布影响 PR 必须在同一个 PR 内提交版本和 release intent，合并后由 Release On Main 自动创建、发布和复验不可变 Release；本机验证以运行中的 generation、当前 provider readiness 和真实用户路径为准。
