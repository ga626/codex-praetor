...
- CodeBuddy validation on 2026-07-07/2026-07-08: the selected HY3 route used model id `hy3`, returned successful smoke output, and the user's CodeBuddy usage detail showed `hy3` calls consuming 0 credits. Tencent Cloud TokenHub CodeBuddy Code official setup also uses `hy3` in `models.json` and `availableModels`. Do not replace it with `auto`; `auto` is blocked because it gives provider-side model choice back to CodeBuddy. Do not default to `hy3-preview-agent`; bundled product configs list `hy3-preview-agent` with `x0.04 credits`, and the user's usage detail showed nonzero credit consumption. For non-interactive readonly file checks on Windows, the reliable combination was `-y --tools Read,Glob`; `--permission-mode plan` either exceeded turns or returned an incorrect file existence result, and `--permission-mode bypassPermissions` returned an incorrect result in the same probe.
