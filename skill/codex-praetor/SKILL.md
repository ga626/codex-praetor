...
4. Codex 顺序检查 `completion.json`、stdout/stderr、worktree diff/status、允许范围、成功 marker 和独立复跑，才记录 `accepted`；只有 `accepted` 可解锁依赖。拒绝、超时、无输出或遗留部分差异均为失败证据，不静默重试、合并或当作成功。
