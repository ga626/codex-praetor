...
Current productization target: **0.16.2-alpha**. This release makes real `code_change` work start from a frozen-commit isolated Git worktree. A result can become fact only when its real diff, scope, immutable files, and Codex's independent verification all pass.
...
[Download 0.16.2-alpha](https://github.com/ga626/codex-praetor/releases/tag/v0.16.2-alpha) · [Chinese installation guide](docs/user/installation.zh.md) · [Chinese troubleshooting guide](docs/user/troubleshooting.zh.md)
...
```powershell
Invoke-WebRequest -Uri "https://github.com/ga626/codex-praetor/releases/download/v0.16.2-alpha/codex-praetor-setup-0.16.2-alpha.zip" -OutFile ".\codex-praetor-setup-0.16.2-alpha.zip"
Expand-Archive .\codex-praetor-setup-0.16.2-alpha.zip .\codex-praetor-setup-0.16.2-alpha
cd .\codex-praetor-setup-0.16.2-alpha
```
...
The 0.16.2-alpha source line records runtime identity alongside logical tasks, immutable worker attempts, evidence, and Codex supervisor verdicts. A worker process exit or same-name tool list is not enough to advance a release; Codex must verify the loaded runtime generation.
